...
export function inspectZip(
  bytes: Uint8Array,
  options?: { countUncompressed?: (name: string) => boolean },
): ZipEntry[] {
  if (bytes.length < 22) {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (!options?.countUncompressed || options.countUncompressed(name)) {
...
export function zipLocalPayload(bytes: Uint8Array, entry: ZipEntry): Uint8Array {
